...
### variables
X = [int(x[:]) for x in open("inputs.txt", "r").readlines()]
Y = [int(x[:]) for x in open("outputs.txt", "r").readlines()]
...
class Vector(object):
	def __init__(self, x, y, angle_with_x = None):
		self.x = x
		self.y = y
		self.pos = (x, y)

		if angle_with_x is not None:
			self.slope = tan(angle_with_x)
	...
	def __mul__(self, other):
		pos_self = np.array(list(self.pos))
		pos_other = np.array(list(other.pos))

		return list(np.cross(pos_self, pos_other))
